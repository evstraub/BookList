...
class Book {
  ...
  get info() {
    return `${title} by ${author}, ${pageCount} pages, ${
      haveRead === true ? "Finished" : "Not Finished"
    }`;
  }
}
...
function showBook(arrayIndex) {
  ...
  p.textContent = `${myLibrary[arrayIndex].title} by ${
    myLibrary[arrayIndex].author
  }, ${myLibrary[arrayIndex].pageCount} pages, ${
    myLibrary[arrayIndex].haveRead === true ? "Finished" : "Not Finished"
  }`;
